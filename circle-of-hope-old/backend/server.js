...
/* AUTH */
app.post("/specialist-advice", (req, res) => {
  ...
  const fallbackAdvice = [
    {
      keywords: ["noise", "loud", "sound", "sensory", "headphones"],
      title: "Sensory Sensitivity",
      answer:
        "Your question seems related to sensory sensitivity. Try to reduce loud or sudden sounds, prepare the child before noisy places, and provide calming tools such as headphones or a quiet space.",
    },
    {
      keywords: ["speech", "talk", "communication", "language", "words"],
      title: "Communication Support",
      answer:
        "Your question seems related to communication. Try using simple words, visual cards, gestures, and give the child enough time to respond. Repeating calmly can also help.",
    },
    {
      keywords: ["tantrum", "crying", "anger", "meltdown", "behavior", "upset"],
      title: "Behavior Support",
      answer:
        "Your question seems related to behavior support. Try to understand the reason behind the behavior, stay calm, reduce triggers, and use a predictable routine.",
    },
    {
      keywords: ["routine", "schedule", "change", "transition"],
      title: "Daily Routine",
      answer:
        "Your question seems related to routine. A clear daily schedule, visual reminders, and preparing the child before changes can help reduce stress.",
    },
    {
      keywords: ["school", "teacher", "class", "learning", "study"],
      title: "School Support",
      answer:
        "Your question seems related to school support. Clear instructions, visual aids, teacher cooperation, and a calm classroom environment can help the child learn better.",
    },
  ];
  ...
  words.forEach((word) => {
    conditions.push(
      "LOWER(title) LIKE ? OR LOWER(category) LIKE ? OR LOWER(content) LIKE ? OR LOWER(keywords) LIKE ?"
    );
  ...
  if (conditions.length === 0) {
    return res.status(400).json({ message: "Please write a clearer question" });
  }
  ...
  db.query(sql, values, (err, results) => {
    if (err) {
      console.error("Error searching autism guide:", err);
    ...
    const matchedFallback = fallbackAdvice.find((item) =>
    ...
    if (matchedFallback) {
      return res.json({
        ...
        answer: matchedFallback.answer,
      });
    }

    return res.json({
      source: "Fallback Advice",
      title: "General Autism Support",
      category: "General Support",
      answer:
        "This question may need more specific support. Try observing the child’s behavior, identifying possible triggers, using calm communication, and asking a specialist for professional guidance if the situation continues or becomes serious.",
    });
  });
});
...
app.get("/anonymous-questions/:id/comments", (req, res) => {
  ...
  const sql = `
    SELECT 
      ...
      c.created_at,
      u.name
    FROM anonymous_question_comments c
  ...
  db.query(sql, [questionId], (err, results) => {
    if (err) {
      console.error("Error fetching anonymous question comments:", err);
      return res.status(500).json({
        message: "Server error",
        error: err.message,
      });
...
app.post("/anonymous-questions/:id/comments", (req, res) => {
  const questionId = req.params.id;
  const { userId, comment } = req.body;

  if (!userId || !comment || !comment.trim()) {
    return res.status(400).json({
      message: "Missing userId or comment",
    });
  }

  const sql = `
    INSERT INTO anonymous_question_comments 
    (question_id, user_id, comment)
    VALUES (?, ?, ?)
  `;

  db.query(sql, [questionId, userId, comment], (err, result) => {
    if (err) {
      console.error("Error adding anonymous question comment:", err);
      return res.status(500).json({
        message: "Server error",
        error: err.message,
      });
